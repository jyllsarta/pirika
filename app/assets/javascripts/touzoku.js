...
function update() {
	try {
		...
		console.log("盗賊ちゃんクリッカーを終了するよ");
		clearInterval(update_interval);
		clearInterval(save_interval);
		clearInterval(golden_interval);
...
//10ミリ秒ごとに少しずつ動かす
update_interval = window.setInterval(update, UPDATE_FREQ_MS);
save_interval = window.setInterval(save, SAVE_FREQ_MS);
golden_interval = window.setInterval(golden_cookie_check, 1000);
message_check_interval = window.setInterval(message_check, 1000);
